...
if ($sReqData) {
    ...
    if ($errCode == 0) {
        ...
        $requestURL="http://localhost/cdapp/functions/zztk/php/reply.php?type={$reqMsgType}&event={$reqEvent}&key={$reqEventKey}&text={$reqContent}";
        $myContent = file_get_contents($requestURL);
	if(empty($myContent)||is_null($myContent)){die();}
        $shortContent = str_replace("\n", '><', str_replace("\n\n", "\n", $myContent));
        journal('zztk', "User: <{$reqFromUserName}>\r\nReceive: <{$reqContent}>\r\nReply: <{$shortContent}>");
        $sRespData = "<xml>
